...
public interface WorkloadStatMapper extends BaseMapper<WorkloadStat> {
    ...
    @Select("SELECT * FROM workload_stat WHERE stat_date = #{date} AND deleted = 0")
    List<WorkloadStat> findByDate(@Param("date") LocalDate date);

    @Select("SELECT work_type, SUM(prescription_count) as count FROM workload_stat WHERE stat_date BETWEEN #{start} AND #{end} AND deleted = 0 GROUP BY work_type")
    List<Map<String, Object>> aggregateByType(@Param("start") LocalDate start, @Param("end") LocalDate end);

    @Select("SELECT * FROM workload_stat WHERE operator_id = #{operatorId} AND stat_date = #{date} AND deleted = 0")
