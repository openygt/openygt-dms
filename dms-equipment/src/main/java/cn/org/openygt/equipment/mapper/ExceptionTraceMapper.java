...
public interface ExceptionTraceMapper {
    ...
    @Select("<script>"
            ...
            + "</script>")
    List<ProdExceptionLog> list(@Param("keyword") String keyword, @Param("exceptionType") String exceptionType,
    ...
    ProdExceptionLog getById(@Param("id") Long id);
    ...
    @Select("<script>"
            + "SELECT handle_status as name, COUNT(*) as value FROM prod_exception_log WHERE is_deleted = 0 "
            + "<if test='dateStart != null and dateStart !=\"\"'> AND created_at &gt;= #{dateStart}</if>"
            + "<if test='dateEnd != null and dateEnd !=\"\"'> AND created_at &lt;= #{dateEnd}</if>"
            + " GROUP BY handle_status"
            + "</script>")
    List<Map<String, Object>> statByStatus(@Param("dateStart") String dateStart, @Param("dateEnd") String dateEnd);
}
