...
public interface MedicineGroupMapper extends BaseMapper<MedicineGroup> {
    ...
    @Select("SELECT * FROM medicine_group WHERE prescription_no = #{prescriptionNo} AND deleted = 0 ORDER BY group_type, sort_order")
    List<MedicineGroup> findByPrescriptionNo(@Param("prescriptionNo") String prescriptionNo);

    @Select("SELECT * FROM medicine_group WHERE prescription_no = #{prescriptionNo} AND group_type = #{groupType} AND deleted = 0 ORDER BY sort_order")
