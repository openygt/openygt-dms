...
面向中药房汤剂煎煮场景的全流程智能管理系统，覆盖从处方接收到患者取药的完整链路。本系统可用于中药饮片企业代煎服务投标，如需方案支持，请访问 [县域中药饮片代煎代配 赋能成长平台](https://dj.openygt.org.cn)。

> 内置 IoT 网关，支持通过 MQTT 协议对接主流煎药机（需设备支持 MQTT 3.1/3.1.1 协议）、包装机、标签打印机，实现设备状态实时监控、故障告警、产量统计。
...
| 后端 | Spring Boot + MyBatis-Plus | Spring Boot 2.7.18（升级 3.x 规划中，详见路线图） |
| 数据库 | MySQL | 8.0+ |
| 缓存 | Redis | 7+ |
...
> 某县域医共体统一建设共享中药房，为辖区内 8 家乡镇卫生院、2 家社区卫生服务中心提供集中煎药与配送服务，日处理处方 200-500 方。解决"多机构药事能力共享与基层配送"痛点。

**系统主界面**

![山东某县医共体共享中药房主界面](images/实施案例-某医共体共享中药房.png)
...
> 某县中医院药剂科下设智慧药房，承担本院门诊代煎与住院病区代煎服务，日处理处方 100-300 方，支持急煎，与 HIS 深度集成、医保 DRG/DIP 结算。解决"门诊代煎量大、住院直送病区流程复杂，偶有急煎需快速响应"痛点。

**系统主界面**

![河北某市中医院智慧药房主界面](images/某中医院智慧中药房.png)

**核心功能模块（二级）**

- **处方管理**：门诊处方接入、住院医嘱接入、HIS 深度集成、处方前置审核、住院医嘱审核、协定方管理
- **煎药管理**：门诊代煎、住院代煎、急煎通道、煎煮方案配置、泡药/煎药/包装工序管理、设备监控
- **住院配送**：病区护士站配送、住院按袋发货、住院全部发货、配送状态跟踪
- **药品管理**：本院药事委员会目录、门诊药房/住院药房/中心药房一院一库、药品匹配、配伍禁忌
- **质量管理**：全流程追溯（到病区/床位）、温度曲线分析、留样管理、工序时效预警
- **医保对接**：医保/DRG/DIP 结算、医院统一收费窗口、费用实时同步
- **设备管理**：煎药机/包装机档案、设备监控、维护计划、维修记录
- **报表统计**：工作量报表、药品分类报表、处方贴数报表、住院代煎统计、门诊代煎统计
- **大屏展示**：调剂大屏、复核大屏、煎药大屏、包装大屏、住院配送大屏
- **系统管理**：用户与权限管理、角色管理、消息通知、处方提示规则、默认参数

---

### 案例三：黑龙江某市第三方煎药中心

> 黑龙江某市第三方煎药中心，面向多家医院与诊所提供代煎代配服务，日处理处方 300-800 方，覆盖接单、调剂、煎煮、包装、发货全流水线。解决"多客户独立运营"痛点。
...
> 某连锁中医品牌旗下 8 家门店共用一套煎药中心，日处理处方 100-400 方，支持多门店订单聚合、统一调剂煎煮、按门店分批发货。解决"多门店药事协同与品控统一"痛点。

**系统主界面**

![河北安国市某连锁中医馆中药房主界面](images/连锁中医馆.png)
...
> 某民营中医馆煎药室，日处理处方 20-50 方，配置 3 台煎药机，覆盖处方录入、审方、煎药、库存、PDA 全流程作业。解决"小成本全流程管理"痛点。

**系统主界面**

![河北石家庄市某中医馆煎药室主界面](images/某中医馆煎药室.png)
...
| 维度 | 共享中药房 | 县中医院智慧药房 | 第三方煎药中心 | 连锁中医馆中药房 | 中医馆煎药室 |
|:---|:---|:---|:---|:---|:---|
| **一句话** | 牵头医院给下级卫生院共享药事能力 | 一家县中医院内部的药事服务单元，门诊代煎+住院代煎，HIS深度集成 | 独立企业给多家医疗机构提供代煎调配服务 | 连锁体系内各门店的药事服务单元（总部统筹或门店自管） | 单体医馆内部自用的煎药服务 |
| **组织关系** | 医共体·行政隶属 | 单体医院·内部科室（药剂科） | 第三方·商业合作 | 连锁集团·企业管理 | 单体机构·内部科室 |
| **服务对象** | 医共体内基层成员单位 | 本院门诊患者+住院病区 | 多种签约医疗机构 | 本品牌各门店及到店患者 | 本馆患者 |
| **核心驱动** | 政策驱动：资源下沉、同质化管理 | 临床驱动：门诊代煎量大、住院代煎流程复杂、医保合规结算 | 商业驱动：市场化运营、多客户竞争 | 管理驱动：品牌一致性、服务标准化 | 服务驱动：自给自足、即时交付 |
| **处方来源** | 医共体内统一HIS | 本院HIS（门诊处方+住院医嘱） | 多家异构HIS/互联网医院 | 各门店坐堂医师 | 本馆医师 |
| **处方审核** | 牵头医院执业药师统一审方 | 本院药剂科执业药师审方（门诊前置审核+住院医嘱审核） | 执业药师实质审方（配伍禁忌、用法用量、禁忌症） | 门店执业药师审方；远程审方需符合当地卫健委政策要求 | 本馆执业药师审方（如有）；无药师时需依托总部或第三方审方服务 |
| **药品目录** | 医共体统一目录 | 本院药事委员会制定，与HIS完全一体 | 按客户协议定制 | 连锁总部统一制定 | 医馆自主采购 |
| **库存管理** | 中心药房↔基层药房，多机构联动 | 一院一库：门诊药房+住院药房+中心药房 | 多客户库存隔离，各自独立 | 总部↔门店调拨 | 一仓一药房 |
| **煎药能力** | 日处理 200-500 方 | 日处理 100-300 方，支持急煎通道 | 日处理 300-800 方，支持多剂型 | 门店自管：各1-2台；总部统筹：中心集中多台 | 1-2台煎药机，日处理数十方 |
| **配送方式** | 到成员单位药房/卫生站（由成员单位发药给患者），部分支持直配患者 | 门诊：患者自取/快递到家；住院：配送到病区护士站 | 到签约机构+第三方到家 | 到门店+快递/跑腿到家 | 患者自取为主，部分提供配送 |
| **结算方式** | 医共体内部核算+医保统付 | 走医院统一收费窗口，深度嵌入医保/DRG/DIP | 多客户账套+独立计费 | 企业内部成本分摊+患者自费 | 馆内统一核算 |
| **医保对接** | 通过牵头医院统一对接 | 直接走本院医保系统，最完整 | 需独立申请或依托客户医保 | 大多自费，部分项目医保 | 部分项目医保 |
| **追溯体系** | 处方→患者（处方ID、患者信息、基层机构） | 处方→患者→病区（处方ID、患者信息、病区/床位、煎药批次） | 处方→客户机构→患者（处方ID、客户机构、患者信息、煎药批次） | 处方→门店→患者（处方ID、门店、患者信息） | 处方→患者（处方ID、患者信息） |
| **监管框架** | 《紧密型县域医共体药事管理指南》+地方医共体管理办法 | 《医疗机构药事管理规定》+《医院中药饮片管理规范》（要求最严格） | 《药品管理法》+《医疗机构中药煎药室管理规范》+地方代煎服务监管办法 | 《医疗机构药事管理规定》或《中医诊所管理暂行办法》（视资质类型）+企业内部质控体系 | 《医疗机构管理条例》+《中医诊所管理暂行办法》 |
| **人员配置** | 执业药师+审方药师+调剂员+煎药员+质控 | 药剂科主任+审方药师+调剂员+煎药员+质控（配置最完整） | 审方药师+调剂员+煎药员+质控+客服 | 执业药师+调剂员+煎药员（总部统筹模式下人员更集中） | 执业药师+调剂员兼煎药员（1-2人身兼多职） |
| **规模** | 中大型 | 中大型 | 中大型 | 中小型（门店自管）或中大型（总部统筹） | 微型 |
...
| 版本 | 协议 | 适用场景 |
|------|------|----------|
| 开源版 | [AGPL-3.0](LICENSE) | 免费用，若基于本系统向第三方提供SaaS/云服务，需开源修改后的代码 |
...
| 医疗机构闭源部署、不公开修改的代码 | 商业授权 | 联系 luobin@openygt.org.cn |
| 不确定 | 联系我们 | — |
...
**商务合作**：luobin@openygt.org.cn ｜ 加微信 **openygt-wx** ｜ 扫二维码加微信
