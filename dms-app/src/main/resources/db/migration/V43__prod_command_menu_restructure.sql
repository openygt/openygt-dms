...
-- --------------------------------------------------
-- 3. 生产指挥子菜单（13项，按最终版方案排序）
--    INSERT IGNORE：若旧记录已在步骤2更新为新 code，则跳过冲突
-- --------------------------------------------------
...
-- --------------------------------------------------
-- 4. 角色菜单绑定（新菜单 + 父目录 prod_command）
-- --------------------------------------------------

-- 4.1 ADMIN 绑定生产指挥全部菜单（含父目录）
INSERT IGNORE INTO sys_role_menu (role_id, menu_id)
SELECT r.id, m.id FROM sys_role r, sys_menu m
WHERE r.role_code = 'ROLE_ADMIN' AND (m.code = 'prod_command' OR m.code LIKE 'cmd_%');

-- 4.2 DIRECTOR 绑定生产指挥全部菜单（含父目录）
INSERT IGNORE INTO sys_role_menu (role_id, menu_id)
SELECT r.id, m.id FROM sys_role r, sys_menu m
WHERE r.role_code = 'ROLE_DIRECTOR' AND (m.code = 'prod_command' OR m.code LIKE 'cmd_%');

-- 4.3 LEADER 绑定生产指挥查看类菜单（不含远程急停、设备联网、设备分组编辑；含父目录）
INSERT IGNORE INTO sys_role_menu (role_id, menu_id)
SELECT r.id, m.id FROM sys_role r, sys_menu m
