...
CREATE TABLE IF NOT EXISTS pda_login_record (
    ...
    online_duration INTEGER,                    -- 在线时长（秒），logout 时计算
...
CREATE INDEX IF NOT EXISTS idx_pda_login_user ON pda_login_record(user_id);
CREATE INDEX IF NOT EXISTS idx_pda_login_device ON pda_login_record(device_id);
CREATE INDEX IF NOT EXISTS idx_pda_login_status ON pda_login_record(status);
CREATE INDEX IF NOT EXISTS idx_pda_login_time ON pda_login_record(login_time);
...
CREATE INDEX IF NOT EXISTS idx_pda_photo_task ON pda_review_photo(task_id);
CREATE INDEX IF NOT EXISTS idx_pda_photo_prescription ON pda_review_photo(prescription_id);
CREATE INDEX IF NOT EXISTS idx_pda_photo_time ON pda_review_photo(review_time);
...
CREATE TABLE IF NOT EXISTS pda_operation_log (
    ...
    request_time    INTEGER,                    -- 请求耗时（毫秒）
    oper_time       DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
    tenant_id       VARCHAR(32)     DEFAULT 'default',
    created_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_pda_log_user ON pda_operation_log(user_id);
CREATE INDEX IF NOT EXISTS idx_pda_log_device ON pda_operation_log(device_code);
CREATE INDEX IF NOT EXISTS idx_pda_log_task ON pda_operation_log(task_id);
CREATE INDEX IF NOT EXISTS idx_pda_log_type ON pda_operation_log(oper_type);
CREATE INDEX IF NOT EXISTS idx_pda_log_result ON pda_operation_log(oper_result);
CREATE INDEX IF NOT EXISTS idx_pda_log_time ON pda_operation_log(oper_time);
