...
public interface TaskMapper extends BaseMapper<Task> {
     ...
     */
    @Select("<script>" +
            "SELECT DATE(t.created_at) as statDate, COUNT(*) as totalTasks, " +
            ...
            "<if test='hospitalId != null'>AND p.hospital_id = #{hospitalId}</if> " +
            "GROUP BY DATE(t.created_at) ORDER BY statDate DESC" +
            "</script>")
    List<Map<String, Object>> selectDailyCapacity(@Param("startDate") String startDate,
