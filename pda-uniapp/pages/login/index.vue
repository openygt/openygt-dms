<template>
  <view class="login-container">
    ...
    <view class="login-tabs">
      <view class="tab-item" :class="{ active: loginType === 'scan' }" @click="loginType = 'scan'">扫码登录</view>
      <view class="tab-item" :class="{ active: loginType === 'account' }" @click="loginType = 'account'">账号登录</view>
    </view>
    ...
    <view class="login-form" v-if="loginType === 'scan'">
      <view class="scan-panel" @click="handleScan">
        ...
        <text class="scan-text">{{ scanCode ? scanCode : '请扫描员工条码' }}</text>
      </view>
      <view class="form-item">
        <text class="label">设备编码</text>
        <input class="input" v-model="form.deviceCode" placeholder="请扫描设备码" />
      </view>
      <button class="login-btn" :loading="loading" :disabled="loading || !scanCode" @click="handleScanLogin">登录</button>
    </view>
    ...
    <view class="login-form" v-else>
      <view class="form-item">
        ...
        <input class="input" v-model="form.userCode" placeholder="请扫描或输入工号" confirm-type="next" @confirm="focusPassword" />
      </view>
      <view class="form-item">
        <text class="label">密码</text>
        <input ref="passwordRef" class="input" v-model="form.password" password placeholder="请输入密码" confirm-type="done" @confirm="handleLogin" />
      </view>
      <view class="form-item">
        <text class="label">设备编码</text>
        <input class="input" v-model="form.deviceCode" placeholder="请扫描设备码" />
      </view>
      <button class="login-btn" :loading="loading" :disabled="loading" @click="handleLogin">登录</button>
    </view>
